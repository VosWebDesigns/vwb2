    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-8 backdrop-blur-sm" role="dialog" aria-modal="true">
      <div className="max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-3xl border border-white/10 bg-[#07111f] shadow-2xl">
        <div className="flex items-center justify-between gap-4 border-b border-white/10 p-5">
          <div>
            <p className="text-xs font-black uppercase tracking-[.2em] text-[var(--accent)]">Afbeelding kiezen</p>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="p-5">
          <TabsList className="grid h-auto w-full grid-cols-3 bg-black/30 text-slate-300">
            <TabsTrigger value="upload" className="gap-2 data-[state=active]:bg-[var(--accent)] data-[state=active]:text-black"><Upload size={15} /> Upload</TabsTrigger>
            <TabsTrigger value="gallery" className="gap-2 data-[state=active]:bg-[var(--accent)] data-[state=active]:text-black"><ImagePlus size={15} /> Galerij</TabsTrigger>
            <TabsTrigger value="url" className="gap-2 data-[state=active]:bg-[var(--accent)] data-[state=active]:text-black"><Link size={15} /> URL</TabsTrigger>
          </TabsList>

          <TabsContent value="upload" className="mt-5">
            <label className="flex min-h-56 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-white/20 bg-black/20 p-8 text-center transition hover:border-[var(--accent)]/70">
              {uploading ? <Loader2 className="mb-3 animate-spin text-[var(--accent)]" /> : <Upload className="mb-3 text-[var(--accent)]" />}
          ...
          <TabsContent value="gallery" className="mt-5 space-y-4">
            ...
            {galleryLoading ? (
              ...
              <div className="grid max-h-[52vh] grid-cols-2 gap-3 overflow-y-auto pr-1 md:grid-cols-3 lg:grid-cols-4">
                {gallery.map((item) => (
                  <button key={`${item.bucket}:${item.path}`} type="button" onClick={() => selectImage(item)} className="overflow-hidden rounded-2xl border border-white/10 bg-black/20 p-2 text-left transition hover:border-[var(--accent)]">
          ...
          <TabsContent value="url" className="mt-5 space-y-4">
            ...
            <Button type="button" onClick={selectManualUrl} className="bg-[var(--accent)] text-black hover:bg-[#7dd3fc]">Selecteer URL</Button>
          </TabsContent>
        </Tabs>
      </div>
    </div>
