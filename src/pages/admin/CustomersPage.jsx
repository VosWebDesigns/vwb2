...
const inputClass = 'w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-[#38bdf8] focus:ring-2 focus:ring-[#38bdf8]/20';
  ...
  return (
    <div className="space-y-8 pb-24">
      ...
      <header className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[.22em] text-[#38bdf8]">Vos Admin CRM</p>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button onClick={startNewCustomer} className="gap-2 bg-[#38bdf8] text-black hover:bg-[#0ea5e9]"><Plus size={16} /> Nieuwe klant</Button>
        </div>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
        <Card className="border-white/10 bg-[#111827]">
          ...
          <CardContent>
            <form onSubmit={saveCustomer} className="space-y-4">
              ...
              <Button type="submit" className="w-full gap-2 bg-[#38bdf8] text-black hover:bg-[#0ea5e9]"><Save size={16} /> Opslaan</Button>
            </form>
          </CardContent>
        </Card>
        ...
        <div className="space-y-4">
          ...
          {loading ? (
            <div className="rounded-2xl border border-white/10 bg-[#111827] p-8 text-center text-slate-400">Klanten laden…</div>
          ) : filteredCustomers.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-white/10 bg-[#111827] p-12 text-center text-slate-500">
            ...
            <div className="grid gap-3">
              {filteredCustomers.map((customer) => (
                <article key={customer.id} className="rounded-2xl border border-white/10 bg-[#111827] p-5">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <button type="button" onClick={() => editCustomer(customer)} className="text-left">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="rounded-full border border-[#38bdf8]/30 bg-[#38bdf8]/10 px-3 py-1 text-xs font-bold uppercase tracking-[.14em] text-[#38bdf8]">{STATUS_OPTIONS.find((option) => option.value === customer.status)?.label || customer.status}</span>
                      </div>
