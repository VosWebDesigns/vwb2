...
const DashboardPage = () => {
  ...
  const StatCard = ({ title, value, icon: Icon, href, accent }) => <Card className="bg-[#111827] border-gray-800"><CardHeader className="flex flex-row items-center justify-between pb-2"><CardTitle className="text-sm font-medium text-gray-400">{title}</CardTitle><Icon className={`h-4 w-4 ${accent ? 'text-[#38bdf8]' : 'text-slate-400'}`} /></CardHeader><CardContent><div className="text-2xl font-bold text-white">{loading ? '-' : value}</div>{href && <Button variant="link" className="px-0 text-[#38bdf8] h-auto mt-2 text-xs flex items-center gap-1" onClick={() => navigate(href)}>Beheren <ArrowRight size={12} /></Button>}</CardContent></Card>;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="col-span-4 bg-[#111827] border-gray-800"><CardHeader><CardTitle className="text-white">Recente activiteit</CardTitle></CardHeader><CardContent><div className="space-y-4">{activities.length === 0 && <p className="text-gray-500 text-sm">Nog geen activiteit.</p>}{activities.map((a) => { const route = getTargetRoute(a); return <div key={a.id} onClick={() => route && navigate(route)} className={`flex items-center gap-4 text-sm border-b border-gray-800 pb-3 last:border-0 ${route ? 'cursor-pointer hover:bg-[#222]' : ''} p-2 rounded`}><div className="h-2 w-2 rounded-full bg-[#38bdf8]" /><div className="flex-1"><p className="text-gray-300">{formatActivityText(a)}<span className="text-xs text-gray-500 ml-2">{a.user_id === user?.id ? '• door jou' : '• andere gebruiker'}</span></p></div><span className="text-gray-500 text-xs">{formatDateTimeNL(a.created_at)}</span></div>; })}</div></CardContent></Card>

        <Card className="col-span-3 bg-[#111827] border-gray-800"><CardHeader><CardTitle className="text-white">Snelle acties</CardTitle></CardHeader><CardContent className="grid gap-3"><Button onClick={() => navigate('/admin/documents?type=quote')} className="h-12 justify-start gap-3 bg-[#38bdf8] text-black hover:bg-[#0ea5e9]"><Plus size={18} /> Nieuwe offerte</Button><Button onClick={() => navigate('/admin/documents?type=invoice')} variant="outline" className="h-12 justify-start gap-3 border-gray-700 text-gray-300"><ReceiptText size={18} className="text-[#38bdf8]" /> Nieuwe factuur</Button><Button onClick={() => navigate('/admin/customers')} variant="outline" className="h-12 justify-start gap-3 border-gray-700 text-gray-300"><Users size={18} className="text-[#38bdf8]" /> Nieuwe klant</Button><Button onClick={() => navigate('/admin/projects?new=true')} variant="outline" className="h-12 justify-start gap-3 border-gray-700 text-gray-300"><FolderKanban size={18} className="text-[#38bdf8]" /> Nieuw project</Button><Button onClick={() => navigate('/admin/inbox')} variant="outline" className="h-12 justify-start gap-3 border-gray-700 text-gray-300"><Inbox size={18} className="text-[#38bdf8]" /> Inbox bekijken</Button><Button onClick={() => window.open('/', '_blank')} variant="ghost" className="h-12 gap-2 text-gray-400 hover:text-[#38bdf8]"><Eye size={16} /> Website bekijken</Button></CardContent></Card>
      </div>
    </div>
  );
};
