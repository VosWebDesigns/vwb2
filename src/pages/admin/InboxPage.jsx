  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 px-4 pb-24">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Button onClick={fetchLeads} variant="outline" className="gap-2 border-[rgba(201,169,110,.16)] text-gray-300"><RefreshCw size={16} className={loading ? 'animate-spin' : ''} /> Vernieuwen</Button>
      </div>

      <div className="rounded-2xl border border-[rgba(201,169,110,.09)] bg-[rgba(8,8,18,.82)] p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex gap-2 overflow-x-auto pb-1">
            {['Alle', ...STATUSES].map((status) => <button key={status} onClick={() => setActiveStatus(status)} className={`whitespace-nowrap rounded-full border px-4 py-2 text-sm font-semibold transition ${activeStatus === status ? 'border-[var(--accent)] bg-[var(--accent)] text-black' : 'border-[rgba(201,169,110,.16)] bg-black/20 text-gray-300 hover:border-gray-500'}`}>{status === 'Alle' ? 'Alle' : statusLabel[status]} <span className="ml-1 opacity-70">{counts[status] || 0}</span></button>)}
          </div>
          <label className="relative min-w-0 lg:w-80"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Zoek lead..." className="w-full rounded-xl border border-[rgba(201,169,110,.16)] bg-black/30 py-3 pl-10 pr-4 text-white outline-none focus:border-[var(--accent)]" /></label>
        </div>
      </div>

      <div className="grid gap-4 lg:grid-cols-[1fr_440px]">
        <div className="space-y-3">
          {loading ? <div className="rounded-2xl border border-[rgba(201,169,110,.09)] bg-[rgba(8,8,18,.82)] p-8 text-center text-gray-400">Leads laden…</div> : filteredLeads.length === 0 ? <div className="rounded-2xl border border-dashed border-[rgba(201,169,110,.09)] bg-[rgba(8,8,18,.82)] p-8 text-center text-gray-500">Geen leads gevonden.</div> : filteredLeads.map((lead) => {
            const status = normalizeStatus(lead.status);
            return <button key={lead.id} onClick={() => setSelectedLead({ ...lead, status })} className={`w-full rounded-2xl border p-4 text-left transition ${selectedLead?.id === lead.id ? 'border-[var(--accent)] bg-[var(--accent)]/10' : 'border-[rgba(201,169,110,.09)] bg-[rgba(8,8,18,.82)] hover:border-[rgba(201,169,110,.16)]'}`}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-lg font-bold text-white">{lead.name}</h2><p className="text-sm text-gray-400">{lead.company || lead.email}</p><p className="mt-2 line-clamp-2 text-sm text-gray-300">{lead.message || 'Geen bericht.'}</p>{lead.notes && <p className="mt-2 text-xs text-[var(--accent)]">Notitie aanwezig</p>}</div><div className="flex shrink-0 flex-col gap-2 sm:items-end"><span className={`rounded-full border px-3 py-1 text-xs font-bold ${statusClass[status] || statusClass.nieuw}`}>{statusLabel[status] || status}</span><span className="text-xs text-gray-500">{formatDateTimeNL(lead.created_at)}</span></div></div></button>;
          })}
        </div>

        <aside className={`fixed inset-x-0 bottom-0 z-40 max-h-[88svh] overflow-y-auto rounded-t-3xl border border-[rgba(201,169,110,.09)] bg-[#0b1220] p-5 shadow-2xl lg:sticky lg:top-8 lg:block lg:max-h-[calc(100vh-4rem)] lg:rounded-2xl ${selectedLead ? 'block' : 'hidden'}`}>
          {selectedLead && <div className="space-y-5">
            <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[var(--accent)]">Lead detail</p><h2 className="mt-2 text-2xl font-bold text-white">{selectedLead.name}</h2><p className="text-gray-400">{selectedLead.company || 'Geen bedrijfsnaam'}</p></div><button onClick={() => setSelectedLead(null)} className="rounded-lg p-2 text-gray-400 hover:bg-white/10 hover:text-white lg:hidden"><X size={20} /></button></div>
            <label className="grid gap-2 text-sm font-semibold text-gray-300">Status<select value={normalizeStatus(selectedLead.status)} onChange={(event) => updateStatus(selectedLead.id, event.target.value)} className="rounded-xl border border-[rgba(201,169,110,.16)] bg-black/30 p-3 text-white outline-none focus:border-[var(--accent)]">{STATUSES.map((status) => <option key={status} value={status}>{statusLabel[status]}</option>)}</select></label>
            <div className="grid gap-2 sm:grid-cols-2"><button onClick={createCustomerFromLead} className="inline-flex items-center justify-center gap-2 rounded-xl border border-emerald-400/30 bg-emerald-400/10 px-4 py-3 font-semibold text-emerald-100 hover:bg-emerald-400/20"><UserPlus size={16} /> Klant maken</button><button onClick={createQuoteFromLead} className="inline-flex items-center justify-center gap-2 rounded-xl border border-[var(--accent)]/30 bg-[var(--accent)]/10 px-4 py-3 font-semibold text-[var(--accent)] hover:bg-[var(--accent)]/20"><FileText size={16} /> Offerte maken</button></div>
            <div className="grid gap-2 sm:grid-cols-2"><a href={mailto} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--accent)] px-4 py-3 font-semibold text-black hover:bg-[#6bc5f5]"><Mail size={16} /> Mail klant</a>{selectedLead.phone ? <a href={`tel:${selectedLead.phone}`} className="inline-flex items-center justify-center gap-2 rounded-xl border border-[rgba(201,169,110,.16)] px-4 py-3 font-semibold text-gray-200 hover:border-[var(--accent)]"><Phone size={16} /> Bel</a> : <span className="inline-flex items-center justify-center gap-2 rounded-xl border border-[rgba(201,169,110,.09)] px-4 py-3 text-gray-600"><Phone size={16} /> Geen tel.</span>}</div>
            <div className="grid gap-2"><label className="text-xs font-bold uppercase tracking-[.18em] text-gray-500">Interne notitie</label><textarea value={noteDraft} onChange={(event) => setNoteDraft(event.target.value)} className="min-h-[120px] rounded-xl border border-[rgba(201,169,110,.16)] bg-black/30 p-3 text-white outline-none focus:border-[var(--accent)]" placeholder="Volgende stap, afspraken, belnotities..." /><Button onClick={saveNote} variant="outline" className="gap-2 border-[rgba(201,169,110,.16)] text-gray-200"><Save size={16} /> Notitie opslaan</Button></div>
            ...
            <div className="rounded-2xl border border-[rgba(201,169,110,.09)] bg-black/20 p-4"><p className="mb-2 text-xs font-bold uppercase tracking-[.18em] text-gray-500">Bericht</p><p className="whitespace-pre-wrap text-gray-200">{selectedLead.message || '—'}</p></div>
          </div>}
        </aside>
      </div>
    </div>
...
const Detail = ({ label, value }) => <div className="rounded-xl border border-[rgba(201,169,110,.09)] bg-black/20 p-3"><dt className="text-xs uppercase tracking-[.16em] text-gray-500">{label}</dt><dd className="mt-1 break-words font-semibold text-gray-200">{value || '—'}</dd></div>;
