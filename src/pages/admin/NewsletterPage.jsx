...
import { ArrowDown, ArrowUp, Loader2, Mail, Plus, Save, Send, Trash2 } from 'lucide-react';
...
const NewsletterPage = () => {
  ...
  const updateBlock = (index, field, value) => setSelected((current) => ({
    ...current,
    content_json: {
      blocks: blocks.map((block, blockIndex) => (blockIndex === index ? { ...block, [field]: value } : block)),
    },
  }));
  const addBlock = (type) => setSelected((current) => ({ ...current, content_json: { blocks: [...blocks, blockTemplate(type)] } }));
  const removeBlock = (index) => setSelected((current) => ({ ...current, content_json: { blocks: blocks.filter((_, blockIndex) => blockIndex !== index) } }));
  const moveBlock = (index, direction) => {
    const next = [...blocks];
    const target = index + direction;
    if (target < 0 || target >= next.length) return;
    [next[index], next[target]] = [next[target], next[index]];
    setSelected((current) => ({ ...current, content_json: { blocks: next } }));
  };
  ...
  const renderPreviewBlock = (block, index) => {
    ...
    if (block.type === 'image') return block.url ? <img key={index} src={block.url} alt={block.alt || ''} className="my-5 w-full rounded-2xl border border-white/10" /> : <div key={index} className="my-5 rounded-2xl border border-dashed border-white/20 p-6 text-slate-500">Afbeelding</div>;
      ...
      <div className="grid gap-6 xl:grid-cols-[320px_1fr]">
        ...
        <section className="grid gap-6 2xl:grid-cols-2">
          <div className="space-y-5 rounded-3xl border border-white/10 bg-[#07111f]/80 p-5">
            ...
            <div><Label>Hero image URL</Label><Input value={selected.hero_image_url || ''} onChange={(e) => updateField('hero_image_url', e.target.value)} className="mt-2 bg-[#050b14]" /></div>
            ...
            <div className="space-y-4">
              {blocks.map((block, index) => (
                <div key={`${block.type}-${index}`} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                  ...
                  {block.type === 'image' ? (
                    <div className="grid gap-3 md:grid-cols-2"><Input placeholder="Afbeelding URL" value={block.url || ''} onChange={(e) => updateBlock(index, 'url', e.target.value)} /><Input placeholder="Alt tekst" value={block.alt || ''} onChange={(e) => updateBlock(index, 'alt', e.target.value)} /></div>
                  ) : block.type === 'cta' ? (
          ...
          <div className="rounded-3xl border border-white/10 bg-[#050b14] p-5">
            ...
            <article className="mx-auto max-w-2xl rounded-[1.5rem] border border-white/10 bg-[#0b1524] p-6">
              ...
              {selected.hero_image_url && <img src={selected.hero_image_url} alt="" className="my-6 w-full rounded-2xl border border-white/10" />}
